...
namespace MS.DataAccess
{
    public class DataCommand : IDataCommand
    {
        ...
        public DataCommand CreateCommand(string sqlKey)
        {
            ...
            {
                throw new System.Exception(string.Format("Can not input an empty sqlKey!", sqlKey));
            }
            m_SQLNode = ConfigHelper.GetSQLList().Find(f => f.SQLKey.Trim().ToUpper() == sqlKey.Trim().ToUpper());
            if (m_SQLNode == null)
            {
                throw new System.Exception(string.Format("Don't found SQLKey:{0} configuration!", sqlKey));
            }
        ...
        private DataSet _queryExecute(QueryFilter filter, string defaultSortBy)
        {
            ...
            {
                throw new System.Exception("Default SortBy can not be empty!");
            }
            ...
            {
                throw new System.Exception("Query SQL Script is Error, it should return 2 tables, 1st table is record count, 2rd table is dataresult.");
            }
            int count = 0;
            if (ds.Tables[0].Rows[0][0] == null || !int.TryParse(ds.Tables[0].Rows[0][0].ToString(), out count))
            {
                throw new System.Exception("Query SQL Script is Error, 1st table is record count, it must be integer.");
            }
            return ds;
