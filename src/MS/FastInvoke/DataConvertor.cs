...
namespace MS.FastInvoke
{
    public static class DataConvertor
    {
        ...
        public static T GetValue<T>(object data, string columnName, string propertyName)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    MethodInfo[] ms = new MethodInfo[2] { null, null };
                    //取得隐式转换方法来转换
                    ms[0] = targetType.GetMethod("op_Implicit");
                    //取得显示转换方法来转换
                    ms[1] = targetType.GetMethod("op_Explicit");
                    foreach (var mInfo in ms)
                    {
                        if (mInfo != null)
                        {
                            if (mInfo.ReturnType == targetType)
                            {
                                ParameterInfo[] pars = mInfo.GetParameters();
                                if (pars[0].ParameterType == data.GetType())
                                {
                                    return (T)mInfo.Invoke(null, new object[] { data });
                                }
                            }
                        }
                    }
        ...
        public static object GetValueByType(Type destinationType, object data, string columnName, string propertyName)
        {
            if (data == null || data == DBNull.Value)
            {
                return typeof(DataConvertor).GetMethod("GetDefaultValue").MakeGenericMethod(destinationType).Invoke(null, null);
            }
            try
            {
